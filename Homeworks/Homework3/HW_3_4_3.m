...
function w = softmax_gradient_descent(X,y)
    ...
    while  norm(grad) > 10^-12 && iter < max_its
        ...
        for i=1:100
            a=-y(i)*X(:,i)'*w;
            grad = grad - (1/(1+exp(-a)))*y(i)*X(:,i);           % YOUR CODE GOES HERE
        end
        w = w - alpha*grad;
...
function [X,y] = load_data()
    data = csvread('D:\Courses Files\Machine Learning\Homeworks\Homework3\exercise 3 wrapper and dataset\imbalanced_2class.csv');
    X = data(:,1:end-1);
    y = data(:,end);
end
