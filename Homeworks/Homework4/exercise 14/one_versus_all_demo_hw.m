...
w1 = softmax_newtons_method(X,y1);
w2 = softmax_newtons_method(X,y2);
w3 = softmax_newtons_method(X,y3);
w4 = softmax_newtons_method(X,y4);
W = [w1 w2 w3 w4];
...
function [X,y] = load_data()
    % load data from file
    data = csvread('D:\Courses Files\Machine Learning\Homeworks\Homework4\exercise 14\4class_data.csv');
    X = data(:,1:end - 1);
    y = data(:,end);
    X = [ones(size(X,1),1), X];
    X = X';
